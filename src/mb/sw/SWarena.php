...
class SWarena {
     ...
     * @param int $void
     */
    public function __construct(SWmain $plugin, string $SWname = "mb", int $slot = 0, string $world = "world", int $countdown = 60, int $maxtime = 300, int $void = 0)
    {
        $this->plugin = $plugin;
        $this->SWname = $SWname;
        $this->slot = $slot;
    ...
    public function getState() : string
    {
        ...
        return TextFormat::WHITE . "§aCLICK TO JOIN";
    }
        ...
        switch ($this->GAME_STATE) {
            case SWarena::STATE_COUNTDOWN:
                ...
                if (($this->countdown - $this->time) == 10)
                    $player->addTitle("§aSTART IN", "§e10", 5, 20, 5);
                    $player->getLevel()->addSound((new \pocketmine\level\sound\PopSound($player)), [$player]);
                ...
                if (($this->countdown - $this->time) == 5)
                    $player->addTitle("§aSTART IN", "§e5", 5, 20, 5);

                if (($this->countdown - $this->time) == 4)
                    $player->addTitle("§aSTART IN", "§e4", 5, 20, 5);

                if (($this->countdown - $this->time) == 3)
                    $player->addTitle("§aSTART IN", "§e3", 5, 20, 5);

                if (($this->countdown - $this->time) == 2)
                    $player->addTitle("§aSTART IN", "§e2", 5, 20, 5);

                if (($this->countdown - $this->time) == 1)
                    $player->addTitle("§aSTART IN", "§e1", 5, 20, 5);
            }
    ...
    public function join(Player $player, bool $sendErrorMessage = true) : bool
    {
		...
		$player->getInventory()->setItem(5, Item::get(355, 14, 1)->setCustomName("§cLOBBY"));
				$player->getInventory()->setItem(3, Item::get(339, 0, 1)->setCustomName("§aINFO MLGBLOCK"));
    ...
    private function start() : void
    {
        ...
        foreach ($this->getPlayers() as $player) {
            ...
            $player->addTitle("§aMLGBLOCK" , "§cSTART GO", 10, 30, 10);
                                $player->setGamemode(0);
                        $player->getLevel()->addSound((new \pocketmine\level\sound\PopSound($player)), [$player]);

$player->getInventory()->clearAll();

$player->getInventory()->setItem(0, Item::get(288, 0, 10)->setCustomName("§aJump"));

$WOOL = Item::get(35,0,64);
