        ...
        switch ($cmd = strtolower(array_shift($args) ?? "help")) {
            ...
            case ($cmd === "create" && $sender->isOp()):
                ...
                if ($args_c < 2 || $args_c > 5) {
                    $sender->sendMessage($this->formatUsageMessage("Usage: /" . $commandLabel . " create <MBname> <slots> [countdown=30] [maxGameTime=600]"));
                    return false;
                }
            ...
            default:
                if ($sender->isOp()) {
                    $sender->sendMessage($this->formatUsageMessage("§e-------------------- \n §aCOMMANDS MLGBLOCK BY ProtectorYT \n §e-------------------- \n §c /mb help:§f Help menu \n §c/mb create:§f Create a game \n §c/mb setspawn: §fDetermine spawn players \n §c/mb delete:§f Delete a game \n §c/mb list:§f list games \n §c/mb join:§f Join Game Auto \n §c/mb quit: §fExit the game \n §e--------------- \n §eAfter you're done, write the following on the board {sign} \n §c mb \n namearena \n §e---------------"));
                } else {
